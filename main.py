...
def processDomecam(file=None, file_bias=None, data_dir=None):
    ...
    cc, cjk, sec_per_frame = one(file=file, file_bias=file_bias, D=D, latency=latency, data_dir=data_dir)
    ...
    # создание гамм -------------------------------------------------------------------------
    gammas = two(lambda_, cjk=cjk, D=D, sec_per_frame=sec_per_frame, latency=latency) # монохроматические 
    # добавить потом и для полихроматич случая
