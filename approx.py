...
#     #     errors = np.sqrt(np.diag(pcov))
        if use_windvar:
            ...
            if dome_only != 0:
                all_info_about_layers = [file, star_name, 'dome layer', spectrum, latency_list, popt[0][0], popt[0][1], popt[0][2], popt[0][3], popt[0][4]]
                df = pd.DataFrame([all_info_about_layers], columns = ['file', 'star', 'mode', 'spectrum', 'latency', 'Vx, m/s','Vy, m/s','Cn2', 'z, m', 'var, m/s'])     
            else:
                all_info_about_layers = [file, star_name, spectrum, latency_list, popt[0][0], popt[0][1], popt[0][2], popt[0][3], popt[0][4]]
                df = pd.DataFrame([all_info_about_layers], columns = ['file', 'star', 'spectrum', 'latency', 'Vx, m/s','Vy, m/s','Cn2', 'z, m', 'var, m/s'])     
        else:
            popt = popt.reshape(len(popt)//4, 4)
            if dome_only != 0:
                all_info_about_layers = [file, star_name, 'dome layer', spectrum, latency_list, popt[0][0], popt[0][1], popt[0][2], popt[0][3]]
                df = pd.DataFrame([all_info_about_layers], columns = ['file', 'star', 'mode', 'spectrum', 'latency', 'Vx, m/s','Vy, m/s','Cn2', 'z, m', 'var, m/s'])     
            else:
                all_info_about_layers = [file, star_name, spectrum, latency_list, popt[0][0], popt[0][1], popt[0][2], popt[0][3]]
                df = pd.DataFrame([all_info_about_layers], columns = ['file', 'star', 'spectrum', 'latency', 'Vx, m/s','Vy, m/s','Cn2', 'z, m'])
