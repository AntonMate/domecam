...
#     print(df_ip.to_string(index=False))
    ...
    def one_speckle_fit(initial_params=None, data=None, latency=None, lambda_=None, all_Vx=None, all_Vy=None, all_Cn2_bounds=None, conjugated_distance=None, dome_index=None, use_gradient=None, do_fitting=None, dome_only=None, use_windvar=None, data_dir=None, file=None, file_name=None, star_name=None, spectrum=None): 
        class _g:
...
#     #     errors = np.sqrt(np.diag(pcov))
        if use_windvar:
            ...
            if dome_only != 0:
                all_info_about_layers = [file, star_name, 'dome layer', spectrum, popt[0][0], popt[0][1], popt[0][2], popt[0][3], popt[0][4]]
                df = pd.DataFrame([all_info_about_layers], columns = ['file', 'star', 'mode', 'spectrum', 'Vx, m/s','Vy, m/s','Cn2', 'z, m', 'var, m/s'])     
            else:
                all_info_about_layers = [file, star_name, spectrum, popt[0][0], popt[0][1], popt[0][2], popt[0][3], popt[0][4]]
                df = pd.DataFrame([all_info_about_layers], columns = ['file', 'star', 'spectrum', 'Vx, m/s','Vy, m/s','Cn2', 'z, m', 'var, m/s'])     
        else:
            popt = popt.reshape(len(popt)//4, 4)
            if dome_only != 0:
                all_info_about_layers = [file, star_name, 'dome layer', spectrum, popt[0][0], popt[0][1], popt[0][2], popt[0][3]]
                df = pd.DataFrame([all_info_about_layers], columns = ['file', 'star', 'mode', 'spectrum', 'Vx, m/s','Vy, m/s','Cn2', 'z, m', 'var, m/s'])     
            else:
                all_info_about_layers = [file, star_name, spectrum, popt[0][0], popt[0][1], popt[0][2], popt[0][3]]
                df = pd.DataFrame([all_info_about_layers], columns = ['file', 'star', 'spectrum', 'Vx, m/s','Vy, m/s','Cn2', 'z, m'])
