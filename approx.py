...
def processApprox(cc=None, gammas=None, lambda_=None, D=None, latency=None, sec_per_frame=None, cjk=None, i_p=None, all_Vx=None, all_Vy=None, p0_Cn2=None, conjugated_distance=None, num_of_layers=None, a1=None, dome_num=None):
    print('approxing')
    print(' - initial guess for the parameters:')
    df_ip = pd.DataFrame(i_p, columns = ['Vx, m/s','Vy, m/s','Cn2', 'z, m']) 
    ...
    def gamma_se(X, Y, Vx, Vy, Cn2, z): 
        ...
        uv, lv = find_nearest(a1, z)
        res = gammas[lv] + (z - a1[lv])*((gammas[uv] - gammas[lv])/(a1[uv] - a1[lv]))
        
        res = shift(res, (-Ypix, Xpix), order=1)  
        return res

    def one_speckle_fit(params=None, data=None, lambda_=None, all_Vx=None, all_Vy=None, p0_Cn2=None, conjugated_distance=None, dome_num=dome_num): 
        def _g(M, *args): 
            x, y = M
        ...
        for i in range(len(all_Vx)):
            if  i == dome_num:
                lb2[i] = [all_Vx[i]-0.5, all_Vy[i]-0.5, p0_Cn2[i][0]-0.005, conjugated_distance-0.05]
                ub2[i] = [all_Vx[i]+0.5, all_Vy[i]+0.5, p0_Cn2[i][1]+0.005, conjugated_distance+0.05]
            else:
...
#     #     errors = np.sqrt(np.diag(pcov))
    ...
    plt.imshow(cc)
    plt.title('cc/cjk')
    fit = one_speckle_fit(params=i_p, data=cc, lambda_=lambda_, all_Vx=all_Vx, all_Vy=all_Vy, p0_Cn2=p0_Cn2, conjugated_distance=conjugated_distance, dome_num=dome_num)
